...
public:
    ...
    void showTreeParent(T itm){ //метод вывода пути от заданного элемента до корня.
        BinaryTreeItem<T> finder = findItem(itm);
        while(finder.getParent()!=nullptr){
            BinaryTreeItem<T> parent;
